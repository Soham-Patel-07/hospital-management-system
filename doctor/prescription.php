...
<div class="wrapper col4">
  <div id="container">
    <h1>Add new prescription record</h1>
     ...
     <input type="hidden"  value="<?php echo $_GET[patientid]; ?>"  />
    ...
    <table width="200" border="3">
      <tbody>
        <tr>
          ...
          <td>
		  <?php
    // Assuming you already have a database connection in $con
    ...
    if (mysqli_num_rows($qsql) > 0) {
        ...
        while ($rs = mysqli_fetch_array($qsql)) {
            echo '<option value="' . $rs['patientid'] . '">' . $rs['patientname'] . '</option>';
        }
        echo '</select>';
    } else {
